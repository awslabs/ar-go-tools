...
// ApplyBaseRewrites applies a series of rewrite steps to the packages and all its dependencies.
// The rewrites are:
//
...
func ApplyBaseRewrites(packages []*packages.Package) {
	rewriteCallsToSort(packages)
	rewriteCallsToOnceDoEtc(packages)
}
