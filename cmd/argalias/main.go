...
import (
	...
	"github.com/awslabs/ar-go-tools/analysis/argalias"
...
const usage = ` Perform an argument aliasing analysis on your packages.
Usage:
    argalias [options] <package path(s)>
Examples:
% argalias -config config.yaml package...
`
...
func main() {
	...
	logger.Printf(formatutil.Faint("Argot argalias tool - build " + version))
	...
	results, err := argalias.Analyze(cfg, lp, ptrRes, goPtrRes)
...
func Report(logger *log.Logger, program *ssa.Program, results []argalias.Result) {
	...
	for _, result := range results {
		...
		for entryArg, aliasedArgs := range result.ArgsAliased {
			...
			for _, alias := range aliasedArgs {
				logger.Printf("\tmay be aliased by arg %s\n", alias)
			}
		}
	}
}
