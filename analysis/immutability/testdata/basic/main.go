...
func exNoModStructInitField() {
	...
	trackInt(&x.x) // @ImmutableArg(exNoModStructInitField) // prints 0
}
...
func exModStructFieldRef() {
	...
	x++            // @InvalidWrite(exModStructFieldRef)
...
func exNoModStructFieldRefAlias() {
	...
	trackInt(v2.x) // @ImmutableArg(exNoModStructFieldRefAlias) // prints 1
}
...
func exModStructFieldRefAlias() {
	...
	v2.x = v1.x
	x++            // @InvalidWrite(exModStructFieldRefAlias)
	trackInt(v2.x) // @ImmutableArg(exModStructFieldRefAlias) prints 2
}
...
func exModStructFieldRefAliasInter() {
	...
	v2.x = v1.x
	mod(&x)
	trackInt(v2.x) // @ImmutableArg(exModStructFieldRefAliasInter) // prints 2
}

func exModStructFieldVal() {
	v := t{x: 1} // @Alloc(exModStructFieldVal) // @InvalidWrite(exModStructFieldVal)
	v.x++        // @InvalidWrite(exModStructFieldVal)
...
func exModClosure() {
	...
	f := func() {
		*x = 1 // @InvalidWrite(exModClosure)
	}
	f()
	trackInt(x) // @ImmutableArg(exModClosure) // prints 1
}
...
func exModClosureInter() {
	...
	f()
	trackInt(x) // @ImmutableArg(exModClosureInter) // prints 1
}

func mod(x *int) {
	*x++ // @InvalidWrite(exModInter, exModStructAliasInter, exModStructFieldRefInter, exModStructRefInter, exModStructInter, exModStructFieldRefAliasInter, exModClosureInter)
}
...
func newT(x int) *t {
	return &t{x: x} // @Alloc(exModStructAliasInter, exModStructRefInter) @InvalidWrite(exModStructAliasInter, exModStructRefInter)
}
